...
function BatAttack() {
	...
	if (floor(image_index) == 3) or (floor(image_index) == 9) image_speed = 0;
	
	//How far we have to jump
	var _distanceToGo = point_distance(x,y,xTo,yTo);
	
	//Begin landing end of the animation once we're nearly done
	if (_distanceToGo < 4) and (image_index < 7) image_speed = 1.0;
...
function BatDie() {
	...
	if (image_index + (sprite_get_speed(sprite_index) / game_get_speed(gamespeed_fps)) >= image_number)
	{
		...
		if (lootDrop >= 0) //100%
		{
			instance_create_layer(x, y, "Player", oCCoin);
		}
		if (lootDrop >= 25) //75%
		{
